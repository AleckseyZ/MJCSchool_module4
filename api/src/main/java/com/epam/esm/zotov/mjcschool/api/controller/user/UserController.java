...
public interface UserController extends ReadController<UserDto> {
        ...
        public ListDto<OrderDto> getUsersOrders(@PathVariable @Min(1) long userId,
                        @RequestParam @Min(0) int startPosition, @RequestParam @Min(1) int limit);
        ...
        public OrderDto addOrder(@PathVariable @Min(1) long userId, @RequestParam @Min(1) long certificateId, Principal principal);

        @PostMapping
        @ResponseStatus(HttpStatus.CREATED)
        public UserDto signUp(@RequestParam @NotBlank String username, @RequestParam @NotBlank String password,
                        @RequestParam @NotBlank String repeatPassword);
}